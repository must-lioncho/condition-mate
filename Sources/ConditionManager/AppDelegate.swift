...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private lazy var dashboard = DashboardServer(
        ...
        page: { [weak self] path in
            path.hasPrefix("/breakdown") ? self?.breakdownPage(path) : self?.transcriptPage(path)
        }
    )
    ...
    private func updateStatusTitle() {
        ...
        // Pomodoro-style focus clock: when a goal is actively in progress, the menu bar
        // shows THAT goal's live elapsed time (the same number the dashboard ticks), so
        // you keep a tight, time-boxed focus on the one task you're solving right now.
        // Only one goal can be in_progress at a time (ReviewStore enforces it).
        if let g = reviewStore.goals.first(where: { $0.status == "in_progress" }) {
            button.title = " " + Formatting.clock(reviewStore.effectiveTracked(g))
            return
        }
        if isWorking {

...
final class ConditionDirector {
    ...
    // 전략8 · 보링 로테이션: cumulative-history roster (신선 우대 · 단골 주간 벤치 교대).
    // A soft re-ranker inside every gate's pool — see BoredomRotation.
    private let boredom: BoredomRotation

    init(activity: ActivityMonitor, library: BPMLibrary, audio: AudioEngine,
         prefStore: TrackPreferenceStore, planMap: BGMPlanMap, boredom: BoredomRotation) {
    ...
    private func playOpener() -> Bool {
        refreshPlanSlot()
        boredom.refreshIfNeeded()   // 전략8: seat this week's roster before the first pick
            ...
            if let pick = library.track(
                ...
                penalty: { [prefStore] in
                    prefStore.bpmPenalty(key: $0.url.lastPathComponent, now: now) + self.recencyPenalty($0)
                        + self.boredom.penalty(key: $0.url.lastPathComponent)
                },
    ...
    private func applyTrack(force: Bool) {
        ...
        // 전략8: cross a weekly/biweekly epoch mid-session and the very next selection
        // already ranks by the fresh roster (cheap no-op otherwise).
        ...
        // 전략8: tag the pick's cohort so the /actions page can audit the rotation.
        let key = candidate.url.lastPathComponent
        let cohort = boredom.isBenched(key: key) ? "벤치(풀 소진 예외)"
            : boredom.isFresh(key: key) ? "신선" : "단골"
        logTrack("trackChange", url: candidate.url, title: candidate.title,
                 detail: (force ? "강제 전환 (슬롯/프로필 변경·싫어요·유휴 전환)"
                                : "목표 \(Int(targetBPM))BPM 근접 선곡") + modeBand + " · 보링 \(cohort)")
    }
}

...
final class DashboardServer {
    ...
    private func respond(_ conn: NWConnection, headerText: String, body reqBody: Data) {
        ...
        // 디버그 모드(버그 수집)일 때만: 서버가 받은 모든 요청을 남긴다. 액션 로그는 POST
        // /api/* 만 보므로 페이지 이동·피드 폴링(GET) 전체가 평소엔 기록되지 않는다 —
        // "그 화면에서 뭘 눌렀는데 아무 일도 안 일어났다"를 재현하려면 이쪽이 필요하다.
        // 꺼져 있으면 Bool 하나 읽고 끝난다.
        let capOn = DebugCapture.shared.isOn
        let capT0 = capOn ? Date() : nil
        defer {
            if let t0 = capT0 {
                DebugCapture.shared.http(method: method, path: path,
        ...
        if method == "GET" && (path.hasPrefix("/api/goal/chat2/stream") || path.hasPrefix("/api/actions/stream")) {
            ...
            send(conn, status: "404 Not Found", contentType: "text/plain; charset=utf-8",
                 body: Data("gone — use /api/loop-engineering".utf8), extra: "")
                                      ...
                                      || path.hasPrefix("/api/debug/capture/status")

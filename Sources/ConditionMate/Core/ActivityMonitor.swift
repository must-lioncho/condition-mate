...
final class ActivityMonitor {
    ...
    // 디버그 모드(버그 수집) 전용 키 상세 훅. LOCAL 모니터에서만 호출된다 — 즉 이 앱
    // 자신의 창으로 들어온 키만 넘어간다. 전역 모니터(다른 앱의 키)는 지금까지처럼
    // 개수만 세며 이 훅을 타지 않는다: 앱 밖에서 친 글자는 어떤 모드에서도 기록하지 않는다.
    var localKeySink: ((NSEvent) -> Void)?
    ...
    func start() {
        ...
        // without this, working inside Condition Mate reads as zero activity.
        ...
        if let m = NSEvent.addLocalMonitorForEvents(matching: mask, handler: { [weak self] event in
            self?.record(event)
            if event.type == .keyDown || event.type == .flagsChanged { self?.localKeySink?(event) }
            return event
        }) {
            monitors.append(m)
        }

...
final class TrackPlayStatsStore {
    ...
    private(set) var activeStrategy = 8
    ...
    // 전략7's retrospective, seeded when the 7→8 migration closes it (and on fresh
    // installs).
    private static let strategy7RetroSeed =
        "장소·컨디션 프리셋은 유효했지만 시간이 지루함을 이겼다 — 같은 활성 풀·같은 상위 곡이 2주~한 달 반복되며 '같은 음원만 도는' 피로가 누적. 재생 이력 자체를 1급 신호로 쓰는 보링 로테이션(전략8)으로 확장: 많이 들은 단골은 주 단위 벤치 교대, 신선 곡(추가 28일 이내·청취 20분 미만)은 2주 보호·우선. 장소·플랜 게이트는 풀 경계로 유지."
    ...
    private static let seedStrategies: [BGMStrategy] = [
        ...
        BGMStrategy(id: 7, name: "장소·컨디션", startedAt: "2026-07-24", endedAt: "2026-08-09",
                    ...
                    retro: strategy7RetroSeed),
        BGMStrategy(id: 8, name: "보링 로테이션", startedAt: "2026-08-09", endedAt: "",
                    summary: "지루함(보링)이 1급 신호: 전략 전체 누적 재생 이력으로 신선(추가 28일 이내·청취 20분 미만)/단골 코호트를 나누고, 단골은 매주(월요일 에폭) 많이 들은 절반을 벤치·덜 들은 절반만 활성으로 교대(벤치 곡은 시간이 안 쌓여 다음 주 자연 복귀 — 자기 균형 로테이션), 신선 곡은 2주 에폭 재평가까지 보호·우선 선곡. 소프트 페널티(가상 BPM)라 풀이 벤치로만 차도 침묵 없음. 기존 게이트(폭우>장소>컨텍스트>플랜>모드)는 불변 — 풀 내부 순위만 완전 개편.",
                    retro: ""),
    ]
    ...
    private func seedStrategiesIfNeeded() {
        ...
        strategies = Self.seedStrategies
        // An empty catalog means fresh/reset state: accrue under the newest strategy
        // (a stale activeStrategy from the file has no catalog entry to belong to).
        activeStrategy = Self.seedStrategies.map { $0.id }.max() ?? activeStrategy
        save()
    }
    ...
    private func migrateCatalog() {
        ...
        // 7 → 8 (2026-08-09, 보링 로테이션): close 전략7, seed its 7→8 transition retro,
        // append 전략8, and accrue new playback time under the boredom rotation. The
        // rotation itself reads totals across ALL strategies (BoredomRotation), so the
        // accrual switch does not reset what counts as "많이 들은 곡".
        if !strategies.contains(where: { $0.id == 8 }) {
            if let i = strategies.firstIndex(where: { $0.id == 7 }) {
                if strategies[i].endedAt.isEmpty { strategies[i].endedAt = "2026-08-09" }
                if strategies[i].retro.isEmpty { strategies[i].retro = Self.strategy7RetroSeed }
            }
            if let entry = Self.seedStrategies.first(where: { $0.id == 8 }) {
                strategies.append(entry)
            }
            activeStrategy = 8
            changed = true

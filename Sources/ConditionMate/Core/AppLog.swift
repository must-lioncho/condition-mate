...
//
// Read it with:  tail -f "$(cat ~/.condition-mate/…)"  — or the path AppLog.path prints once.
enum AppLog {
    ...
    // Exposed so readers can slice the file by time (DebugCapture bundles the capture window).
    static let logTimeZone = TimeZone(identifier: "Asia/Seoul") ?? .current
    private static let df: DateFormatter = {
        let f = DateFormatter()
        f.dateFormat = "yyyy-MM-dd HH:mm:ss.SSS"
        f.timeZone = logTimeZone

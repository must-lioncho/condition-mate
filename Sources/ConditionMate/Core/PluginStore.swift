...
final class PluginStore {
    ...
    struct Plugin {
        ...
        // 이 플러그인이 기대는 외부 자격증명·기능 (IntegrationCatalog의 id).
        // 플러그인이 키 목록을 스스로 들고 있지 않고 id로만 가리키는 이유는,
        // 키의 실제 내용(서비스 이름·발급 경로·검사 방법)이 연동 레지스트리
        // 한곳에만 있어야 하기 때문이다 — 여기에 또 적으면 두 벌이 어긋난다.
    ...
    private static let builtins: [Plugin] = [
        ...
        Plugin(id: "slack-translate",
               ...
               folderPath: "", status: .disconnected, detail: "미설치", verifiedAt: nil)
    ]
    ...
    init() {
        ...
        // 목록 순서는 이름 가나다순 — 정의한 순서(기능이 추가된 순서)로 두면 새 것을
        // 넣을 때마다 사용자가 기억한 위치가 흔들린다. 한글 정렬이 필요하므로
        // localizedStandardCompare(로케일 인지)를 쓴다.
    ...
    func pluginsJSON() -> String {
        let items = plugins.map { p -> String in
            ...
            // 연동 id 배열 — 카드가 이 id로 /api/integrations 응답에서 실제 상태를
            // 찾아 그린다 (이름·발급 경로 같은 내용은 여기 싣지 않는다).
            let creds = p.credentials.map { esc($0) }.joined(separator: ",")
            let caps = p.capabilities.map { esc($0) }.joined(separator: ",")
            return "{\"id\":\(esc(p.id)),\"name\":\(esc(p.name)),\"desc\":\(esc(p.desc)),"
                ...
                + "\"credentials\":[\(creds)],\"capabilities\":[\(caps)],"

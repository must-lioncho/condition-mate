...
// which the Condition Manager dashboard renders (원문+번역+슬랙 링크).
...
// WHY. The app (Sources/Slack/SlackHealth.swift) treats 90s of silence as dead and
...
const DATA_DIR = process.env.CM_DATA_DIR || join(homedir(), '.condition-manager');
...
async function processMessage(channel, ts, reactedAt, emoji, source) {
  ...
  try {
    ...
    const resolved = resolvingReaction(msg.reactions);
...
async function retrigger(id, at, patch = {}) {
  try {
    rewriteItem(id, { triggeredAt: at || Math.floor(Date.now() / 1000), ...patch });
    const ok = await setDoneRemote(id, false);
...
async function reconcileRemoved() {
  ...
  for (const o of open.slice(-30)) {
    ...
    try {
      ...
      const resolved = resolvingReaction(reactions);
      const names = [];
      for (const r of reactions) {
